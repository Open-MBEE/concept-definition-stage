# AGENTS.md — working contract for this concept-definition project

This repository holds a **concept-definition mapping** authored with `cds` (the Concept Definition
Stage tool). This file is the vendor-neutral contract for any AI assistant helping build it. It was
written here by `cds init`. (Tool-specific files like `CLAUDE.md` may just point to this one.)
...
- **Definition-first, don't preconclude.** When a classification is uncertain, look up the canonical
  definition (`cds explain <kind>`), show an example, then hand the deciding call to the human. Don't
  lead with your verdict.
...
- **Always fine:** read-only inspection and authoring (`cds new …`, `cds park …`, `cds queue …`,
  `cds explain …`, `cds verify`, `cds compile`).
- **Confirm with the human first:** anything destructive (`cds rm …`), and `git commit` / `git push`.

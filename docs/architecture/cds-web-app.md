...
>
> *(Amendment, live-QA 2026-08-02 — S1.)* The held-out principle extends to **unresolved citations**: a staged record whose project-local citation resolves to nothing (the `UnresolvedCitation` T2 condition) is held at the commit gate — enumerated in the ChangePlan under its own heading — until the source is secured or the approver names it in `include_unverified` (an explicit act, recorded in the audit event). Rationale: the K1 whitelist confines the *served surface*, not an agentic client's own tools (QA Probe A/B); the enforcement point every transport shares is the gate. The conversational escalate-then-stop guard (K5) remains in the AICC loop and stays orthogonal.
...
> **ADR-9 — Two mutation modes: scratch working copies vs the append-only durable record** *(owner-approved P1.5 amendment, 2026-08-02; emerged from LARP user testing — issues #51/#52).*
> **Mode is a property of the store, not the function.** Any **working copy** — the app's session staging `Project`, the CLI user's project dir — is **scratch**: full CRUD is legitimate (create refusing duplicate slugs; edit-in-place; discard/delete) because nothing there is durable; users need real scratchwork or they will bypass the controls. The **durable record** (canonical repo) is only ever **appended to**: new records; *supersession* — a new record carries authored `cds:supersedes`, and the commit gate materializes the inverse marker `cds:supersededBy` on the old record, whose content triples are never touched; *retraction* — appended `cds:retracted true` (+ `cds:retractionReason`), never deletion; waivers; PROV. The **commit is the mode boundary** (§6.4's "the commit is the assertion"): `git commit` for the standalone CLI user (advisory enforcement — warnings, e.g. `cds rm` on a HEAD-committed record), the K2 gate for the app (mechanical enforcement). Same semantics, two enforcement strengths.
> **Representation:** stable record IRIs + materialized markers — no revision IRIs (they would break slug-based link resolution, `DanglingReference` matching, and compile grouping, and prejudge D7). The **current view** (unsuperseded ∧ unretracted; `cds.core.view`) is a *graph query, never a file/directory convention*; compiled briefs render the current view, with a deterministic history appendix available but **off by default** — the model, not any one rendered document, is canon, and the changelog is never lost by not being displayed (git + markers + PROV hold it). Same-slug **revisions** reach canonical only as enumerated, approver-confirmed lines of the commit gate's `ChangePlan`, which is always written as an artifact stamped with the approver IRI and a content hash binding plan → staging state.
> **Lineage note (dsg-ant-private/ant-rdf review):** the lineage holds state in RDF and change in git only — cds *extends* it here. Adopted constraints: lifecycle markers attach at claim level (`cds:Instance` records are reified statements, so no migration when the deferred multi-perspective Characterization machinery lands); scope IRIs minted today must be usable unchanged as named-graph URIs (D7); under multi-perspective, shapes validate per-frame while conflict checks run over the union — two conflicting claims in different frames are a *finding*, never a violation.
...
| `cds_synthesis`, `cds_new`, `cds_edit` | `authoring.create_synthesis`, `create_record`/`edit_record` | **candidate** into staging — never canonical; `cds_new` refuses an existing slug (409 → "use cds_edit"), `cds_edit` refuses an absent one (404) *(ADR-9)* |
| `cds_discard` | `authoring.remove_record`/`remove_parked`/`remove_queue_item`/`remove_tension` (kind-dispatched) | **scratch-only deletion** of a staged candidate or ledger item — can never touch canonical *(ADR-9, P1.5)* |
| `cds_retract` | `authoring.retract_record` | stages an **append-only retraction intent** (`cds:retracted` + reason) against a canonical record; content triples never removed *(ADR-9, P1.5)* |
...
| `cds_waive` | `verify.Waiver` → `waiver_to_graph` | append-only; **T2/T3 only**; refused on a T1; unknown rule names refused |
| `cds_commit` | staging→canonical merge + git + full verify | **gated**: refused unless caller holds `cds-reviewer`; materializes ADR-9's `ChangePlan` (adds / approver-confirmed revisions / supersessions / retractions) |

No `run_python`, `read_file`, `write_file`, `shell`, `http_get`. This table *is* the model's entire reachable surface — `test_mcp_manifest_equals_whitelist` asserts it. **17 tools** (P1.5). Every tool carries a deontic **mode** — `read` / `scratch` (mutable working copy) / `append` (durable-record intent) / `commit` (the boundary) — served in the generated manifest (ADR-9).
